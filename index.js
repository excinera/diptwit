...
if (h) {
 console.log("diptwit " + versionid + " / x. cinera 2k19 / GPL v3");
  ...
  if (!error) {
   ...
   for (var asdf in rssUrls) {
    ...
    .then(function (items) {
     ...
     console.log(items.feedUrl.length);
     console.log(items.feedUrl.length + " RX RSS " + rssUrls[asdf]);
     smeti[asdf] = items['items'].reverse();
     // d && console.log(guids);
     // var i = 0;
     for(var k in smeti[asdf]){
      item = smeti[asdf][k];
      d && console.log(items.feedUrl.length + " ITEM : " + item.isoDate + ", " + item.guid + ": " + item.title + " (" + guids.indexOf(item.guid) + ")");
      if(guids.indexOf(item.guid) == -1) {
       // for(var j = -1; j < 1000*1000*2000*i; j++) {}
       k = smeti[asdf].length;
       var count = 0;
       count += item.content.split(" ").length - 1;
       (d || v) && console.log(items.feedUrl.length + " NEW  : " + now().format("YYYY-MM-DD") + "T" + now().format("HH:mm:ss") + " GUID " + item.guid + " skip: " + skipAllRest + "/" + skipTheRest[items.feedUrl] + " ( " + count + " words)");
       ...
       if (skipAllRest === 0 && skipTheRest[items.feedUrl] === 0) {
        ...
        .then(function (tweet) {
         (d || v) && console.log(items.feedUrl.length + " TWEET: " + now().format("[YYYY-MM-DD") + "T" + now().format("HH:mm:ss]") + " " + tweet.text);
