...
fs.appendFileSync(__dirname + "/data/run.log", "\n" + now().format("[YYYY-MM-DD") + "T" + now().format("HH:mm:ss]") + " STARTED");

// Check for verbose and discord flags, set variables
...
const clientAuth = JSON.parse(fs.readFileSync(__dirname + "/data/clientAuth.json"));
var client = new Twitter(clientAuth);

var guids = JSON.parse(fs.readFileSync(__dirname + "/data/guids.log"));
